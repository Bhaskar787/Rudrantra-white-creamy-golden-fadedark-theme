...
export function MenuDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex" role="dialog" aria-modal="true" aria-label="Menu">
      {/* Backdrop */}
      <div
        className={`absolute inset-0 bg-[#2D2400]/50 backdrop-blur-md transition-opacity duration-300 ${
      ...
      {/* Panel */}
      <div
        className={`relative w-[90vw] md:w-[62%] max-w-4xl h-full bg-[#FFF8F0] flex flex-col shadow-[20px_0_50px_rgba(0,0,0,0.15)] border-r border-[#E8D5B0] transition-transform duration-300 ease-out ${
          isClosing ? '-translate-x-full' : 'translate-x-0 animate-in slide-in-from-left'
        }`}
      >

        {/* Header */}
        <div className="flex items-center justify-between p-5 md:p-6 border-b border-[#E8D5B0]">
        ...
        <div className="flex-1 overflow-y-auto p-5 md:p-12 relative">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 relative z-10">
              ...
              <nav className="flex flex-col gap-3 md:gap-5">
                {['Pooja', 'Rudraksha Bracelet', 'Japa Mala', 'Combination', 'Shaligram', 'Murtis & Yantra'].map((item, i) => (
                  <Link
                    ...
                    className="text-xl sm:text-2xl text-[#2D2400] hover:text-[#D4AF37] transition-colors group flex items-center justify-between border-b border-[#E8D5B0]/40 pb-2 md:pb-3 animate-in fade-in slide-in-from-left-2 fill-mode-both duration-500"
                  >
                    {item}
                ...
                <nav className="flex flex-col gap-3 md:gap-4 text-sm md:text-base">
                  {['Authenticity', 'Book Consultation', 'Custom Build', 'Read our Story'].map((link) => (
                    <Link
                      ...
                      href="#"
                      onClick={() => setIsMenuOpen(false)}
                      className="text-[#6B5A3E] hover:text-[#D4AF37] transition-colors"
                    >
                      {link}
                    </Link>
                  ))}
            ...
            <div className="flex flex-col justify-between mt-4 md:mt-0">
              <div className="rounded-xl overflow-hidden relative group h-40 md:h-80 border border-[#E8D5B0]">
                <img 
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#2D2400] via-transparent to-transparent p-4 md:p-6 flex flex-col justify-end">
