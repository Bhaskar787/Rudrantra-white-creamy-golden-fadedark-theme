...
export function FAQ() {
  ...
  return (
    <section className="py-14 sm:py-20 md:py-24 bg-[#FFF8F0] relative">
      <style>{`
        @keyframes faq-rise {
      ...
      `}</style>
      <div className="absolute inset-0 bg-[#E8D5B0] opacity-5 pointer-events-none" />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 relative z-10">
        <div className="text-center mb-8 sm:mb-10 md:mb-12 max-w-2xl mx-auto">
          ...
          <p className="font-body text-[#6B5A3E] text-sm sm:text-base md:text-lg leading-relaxed px-2">
        ...
        <div className="relative max-w-lg mx-auto mb-6 sm:mb-8">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6B5A3E]/60" />
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Search a question, e.g. shipping, mukhi, care..."
            className="w-full pl-11 pr-4 py-2.5 sm:py-3 rounded-full border border-[#E8D5B0] bg-white text-xs sm:text-sm font-body text-[#2D2400] placeholder:text-[#6B5A3E]/50 focus:outline-none focus:border-gold transition-colors"
          />
        </div>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-3.5 sm:px-5 py-1.5 sm:py-2 rounded-full text-[10px] sm:text-xs font-heading font-bold uppercase tracking-widest border transition-all duration-300 ${
                activeCategory === cat
                  ? 'bg-gold text-[#2D2400] border-gold shadow-[0_0_15px_rgba(201,151,58,0.3)]'
        ...
        <div className="grid md:grid-cols-[1fr_300px] gap-6 sm:gap-8 md:gap-10 items-start">
          <div ref={listRef} className="flex flex-col gap-2.5 sm:gap-3 relative">
            {filtered.length === 0 && (
              <div className="text-center py-12 sm:py-16 text-[#6B5A3E]/70 border border-dashed border-gold/20 rounded-2xl text-sm sm:text-base px-4">
                ...
                >
                  <div
                    ...
                    className={`relative rounded-xl border transition-[transform,box-shadow,border-color] duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                      isOpen
                        ? 'border-gold/50 bg-white shadow-[0_16px_50px_rgba(0,0,0,0.12)]'
                        : 'border-[#E8D5B0] bg-white shadow-[0_6px_20px_rgba(0,0,0,0.08)] hover:border-gold/30'
                    }`}
                    style={{
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3 sm:gap-4">
                        <span
                          className={`hidden sm:flex w-9 h-9 rounded-full items-center justify-center shrink-0 transition-all duration-300 relative ${
                            isOpen ? 'bg-gold text-[#2D2400] scale-110 faq-bead-active' : 'bg-gold/10 text-gold'
                          }`}
                        >
                          <Icon className="w-4 h-4" />
                        </span>
                        <span className={`font-heading text-sm sm:text-base leading-snug transition-colors duration-300 ${isOpen ? 'text-gold' : 'text-[#2D2400]'}`}>
                          {faq.q}
                        </span>
                      </div>
                      <ChevronDown
                    ...
                    >
                      <div className="overflow-hidden">
                        ...
                        >
                          <div className="h-px bg-[#E8D5B0] mb-3 sm:mb-4" />
                          <p className="font-body text-[#6B5A3E] text-sm sm:text-base leading-relaxed">{faq.a}</p>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <aside className="md:sticky md:top-24 md:self-start">
            <div className="bg-white border border-[#E8D5B0] rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 flex flex-col gap-5 sm:gap-6 shadow-xl">
              ...
              <div className="flex flex-col gap-3 pt-4 border-t border-[#E8D5B0]">
                <a href="tel:+919876543210" className="flex items-center gap-3 text-xs sm:text-sm font-body text-[#2D2400] hover:text-gold transition-colors">
                  <Phone className="w-4 h-4 text-gold shrink-0" /> +91 98765 43210
                </a>
                <p className="text-[9px] sm:text-[10px] text-[#6B5A3E]/60 uppercase tracking-widest pl-7">
