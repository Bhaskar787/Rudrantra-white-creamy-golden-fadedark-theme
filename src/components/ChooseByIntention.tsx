...
const tabs = [
  ...
  },
  {
    ...
    detail: 'Invite abundance and financial clarity into daily life. Chosen for their association with steady growth, sound decisions, and long-term security.',
  ...
  },
  {
    ...
    detail: 'Shield against negative energy and instability. Worn as a quiet safeguard through uncertain periods, travel, or high-stress work.',
  ...
  },
  {
    ...
    detail: 'Restore balance in relationships and the home. A traditional choice for easing tension between partners, family, and close relationships.',
  ...
  },
  {
    ...
    detail: 'Support physical vitality and calm the nervous system. Suited to those focused on rest, recovery, and steadier day-to-day energy.',
    image: 'https://himalayarudraksh.online/cdn/shop/files/1-13-mukhi-shiv-shakti-rudraksha-mala-nepal-origin-499218.png?v=1750001216&width=3840',
  },
];
  ...
  return (
    <>
      ...
      <div className="text-center px-4 mb-10 sm:mb-14 md:mb-16">
        <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-display text-gold-gradient mb-2 sm:mb-3">
          Shop by Intention
        </h2>
        <p className="text-[#6B5A3E] font-body text-sm sm:text-base max-w-xl mx-auto leading-relaxed">
...
function IntentionScrollFlow() {
  ...
  return (
    <div ref={containerRef} className="max-w-3xl mx-auto px-4 sm:px-6 pb-16 sm:pb-24 relative">
      <div className="absolute top-1 bottom-1 w-px bg-[#E8D5B0]" style={{ left: '19px' }} aria-hidden>
        <div ref={fillRef} className="absolute top-0 left-0 w-px bg-gold" style={{ height: '0%' }} />
      </div>
          ...
          >
            <span
              className={`inline-block mb-2 px-3 py-1 rounded-full text-xs font-heading font-bold uppercase tracking-widest transition-colors duration-300 ${
                isReached ? 'bg-gold text-white' : 'bg-[#F4EBD8] text-[#6B5A3E]'
              }`}
            >
              Step {i + 1}
            </span>
            <h3
              className={`font-display text-xl sm:text-2xl md:text-3xl mb-2 transition-colors duration-300 ${
                isReached ? 'text-[#2D2400]' : 'text-[#6B5A3E]'
              }`}
            >
              {tab.label}
            </h3>
            <p className="text-[#6B5A3E] font-body text-sm sm:text-base leading-relaxed mb-5 sm:mb-6 max-w-lg">
              {tab.detail}
            </p>

            <div className="relative w-full aspect-[4/5] sm:aspect-[16/9] md:aspect-[16/10] rounded-2xl overflow-hidden border border-[#E8D5B0] shadow-[0_12px_40px_rgba(45,36,0,0.15)]">
              <img src={tab.image} alt={tab.label} className="w-full h-full object-cover" loading="lazy" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/5 to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-7">
                <h4 className="font-display text-xl sm:text-2xl md:text-3xl text-white mb-1">{tab.label}</h4>
                <p className="text-white/80 font-body text-xs sm:text-sm max-w-md">{tab.desc}</p>
              </div>
            </div>

            <a
              href="#"
              className="inline-flex items-center gap-2 mt-5 sm:mt-6 px-5 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-gold to-gold-soft text-white rounded-full font-heading font-bold uppercase tracking-widest text-xs hover:shadow-[0_0_20px_rgba(201,151,58,0.4)] transition-all"
            >
          ...
          <div className="flex flex-row items-center gap-16 xl:gap-24 w-full">
            ...
            <div className="flex-1 max-w-xl w-full">
              <div className="flex gap-6">
                <div className="relative w-px bg-[#E8D5B0] shrink-0" aria-hidden>
                  <div
                      ...
                      >
                        ...
                        <h3
                          className={`font-display text-xl xl:text-2xl transition-all duration-300 ${
                            isActive ? 'text-[#2D2400] opacity-100' : 'text-[#6B5A3E] opacity-60'
                          }`}
                        >
                          {tab.label}
                        </h3>
                        <p
                          className={`font-body text-sm xl:text-base mt-2 leading-relaxed max-w-md transition-all duration-300 ${
                            isActive ? 'text-[#6B5A3E] opacity-100' : 'text-[#6B5A3E]/70 opacity-60'
                          }`}
                        >
                          {tab.detail}
                        </p>
              ...
              <a
                href="#"
                className="inline-flex items-center gap-2 sm:gap-3 mt-6 sm:mt-8 px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-gold to-gold-soft text-white rounded-full font-heading font-bold uppercase tracking-widest text-xs sm:text-sm hover:shadow-[0_0_20px_rgba(201,151,58,0.4)] transition-all"
              >
            ...
            <div className="flex-1 w-full relative z-10 h-[62vh] sm:h-[68vh] xl:h-[74vh] max-h-[760px]">
              {tabs.map((tab, i) => {
                ...
                return (
                  <div
                    key={tab.id}
                    className="absolute inset-0 rounded-2xl xl:rounded-3xl overflow-hidden shadow-[0_20px_60px_rgba(45,36,0,0.25)] border border-[#E8D5B0] will-change-transform"
                    style={{
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 xl:p-8">
                      <span className="text-gold text-xs xl:text-sm font-heading font-bold uppercase tracking-widest">
                        Step {i + 1}
                      </span>
                      <h3 className="font-display text-2xl xl:text-4xl text-white mt-1 mb-2">{tab.label}</h3>
                      <p className="text-white/80 font-body text-sm xl:text-base max-w-md">{tab.desc}</p>
                    </div>
                  </div>
                );
              })}
...
export function ChooseByIntention() {
  return (
    <section
      className="relative border-y border-[#E8D5B0]"
