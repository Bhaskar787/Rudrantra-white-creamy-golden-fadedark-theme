...
function initialsColor(initials: string) {
  const palette = ['bg-[#FFFAF4]', 'bg-[#FAF3E8]', 'bg-[#FFF8F0]'];
...
export function Testimonials() {
  ...
  return (
    <section className="py-14 sm:py-20 md:py-24 bg-[#FFF8F0] relative overflow-hidden">
      {/* Lotus/Prayer overlay — opacity raised so it actually reads, still
          multiply-blended so it works correctly against the light background */}
      <div
        className="absolute inset-0 opacity-[0.35] mix-blend-multiply bg-cover bg-center"
        style={{ backgroundImage: 'url("https://images.unsplash.com/photo-1527489377706-5bf97e608852?auto=format&fit=crop&w=1920&q=80")' }}
      />

      {/* Dark wash behind the heading + intro line only — that block sits
          directly on the photo above, with no card behind it, so it needs
          guaranteed contrast regardless of what part of the image is behind
          it. Fades to transparent by the time the white stats bar begins,
          since that card already supplies its own background. */}
      <div
        className="absolute inset-x-0 top-0 h-[420px] sm:h-[480px] pointer-events-none"
        style={{
          background:
            'linear-gradient(to bottom, rgba(31,23,6,0.7) 0%, rgba(31,23,6,0.4) 55%, rgba(31,23,6,0) 100%)',
        }}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 relative z-10">
        ...
        <div className="text-center mb-10 sm:mb-14 md:mb-16 max-w-3xl mx-auto">
          <span className="text-[9px] sm:text-[10px] md:text-xs font-heading font-bold uppercase tracking-widest text-gold bg-black/25 backdrop-blur-sm border border-gold/40 px-4 sm:px-5 py-1.5 sm:py-2 rounded-full inline-block mb-4 sm:mb-6 shadow-sm">
            Loved Worldwide
          </span>
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-display text-gold-gradient tracking-tight leading-tight px-2 drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)]">
            Voices of Our Community
          </h2>
          <p className="text-[#F5EAD0] font-body text-sm sm:text-base md:text-lg leading-relaxed mt-4 sm:mt-6 px-2 drop-shadow-[0_1px_4px_rgba(0,0,0,0.45)]">
        ...
        {/* Stats Bar */}
        <div className="grid sm:grid-cols-[auto_1fr] gap-6 sm:gap-8 md:gap-14 bg-white/80 backdrop-blur border border-gold/30 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-12 mb-10 sm:mb-14 md:mb-16 shadow-xl">
          <div className="flex flex-col items-center justify-center text-center sm:border-r sm:border-gold/20 sm:pr-8 md:pr-14">
            ...
            <span className="text-[#6B5A3E] font-heading text-[10px] sm:text-xs uppercase tracking-widest">{testimonials.length * 187}+ verified reviews</span>
          </div>
            ...
            {[5, 4, 3, 2, 1].map((star, i) => {
              ...
              return (
                <div key={star} className="flex items-center gap-2.5 sm:gap-4 text-xs sm:text-sm font-heading font-bold">
                  ...
                  <div className="flex-1 h-1.5 rounded-full bg-[#E8D5B0] overflow-hidden border border-gold/10">
          ...
          {filters.map((f) => (
            <button
              ...
              className={`px-3.5 sm:px-5 py-2 sm:py-2.5 rounded-full text-[10px] sm:text-xs font-heading font-bold uppercase tracking-widest transition-all border ${
                activeFilter === f
                  ? 'bg-gold text-[#2D2400] border-gold shadow-sacred-glow'
                  : 'bg-[#FAF3E8] text-gold/70 border-gold/30 hover:border-gold hover:text-gold'
              }`}
            >
              {f}
            </button>
            ...
            <CarouselContent className="-ml-4 sm:-ml-6">
              {filtered.map((t, i) => (
                <CarouselItem key={i} className="pl-4 sm:pl-6 sm:basis-1/2 lg:basis-1/3">
                  <div className="flex flex-col h-full bg-white/60 backdrop-blur-md border border-gold/30 rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 hover:bg-white/80 hover:border-gold/60 transition-all duration-300 shadow-lg relative group">
                    ...
                    <div className="flex items-start justify-between mb-4 sm:mb-6 relative z-10">
                      <div className="flex items-center gap-3 sm:gap-4">
                        <div className="relative">
                          ...
                          {t.verified && (
                            <div className="absolute -bottom-1 -right-1 bg-[#FFF8F0] rounded-full p-0.5">
                              <BadgeCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gold fill-[#FFF8F0]" />
                            </div>
                          )}
                        </div>
                        <div>
                          <span className="font-heading font-bold text-sm sm:text-base text-[#2D2400] block mb-0.5">{t.name}</span>
                    ...
                    <p className="text-[#6B5A3E] font-body text-sm sm:text-base leading-relaxed line-clamp-5 flex-1 mb-4 sm:mb-6">{t.text}</p>
                    ...
                    <div className="flex flex-col gap-2.5 sm:gap-3 pt-4 sm:pt-5 border-t border-gold/20">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-[#6B5A3E]/60 font-body text-[11px] sm:text-xs">{t.date}</span>
          ...
          {filtered.length > 0 && (
            <div className="flex items-center justify-center gap-3 sm:gap-6 mt-8 sm:mt-12">
              <button
                ...
                className="w-10 h-10 sm:w-12 sm:h-12 rounded-full border border-gold/40 bg-[#FAF3E8] flex items-center justify-center text-gold hover:bg-gold hover:text-[#2D2400] transition-all shadow-md"
              >
              ...
              <button
                ...
                className="w-10 h-10 sm:w-12 sm:h-12 rounded-full border border-gold/20 bg-[#FFF8F0] flex items-center justify-center text-gold/60 hover:text-gold hover:border-gold/50 transition-colors ml-2 sm:ml-4"
              >
