...
export function TrustPaymentBar() {
  return (
    <section className="relative bg-[#FFF8F0] border-y border-gold/20 overflow-hidden">
        ...
        <div className="relative border-y border-gold/30 bg-[#ffeacc] overflow-hidden shadow-sm">
          ...
          <div className="flex animate-marquee whitespace-nowrap items-center py-3">
            {[...paymentMethods, ...paymentMethods].map((method, i) => (
              <div
                key={i}
                className="flex items-center gap-2 shrink-0 mx-2.5 px-4 py-1.5 rounded-lg text-[#2D2400] font-heading text-xs tracking-wide"
              >
                {method.icon ? (
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 flex flex-col items-center text-center border-b border-gold/10">
        ...
        <div className="flex items-center gap-3">
          <span className="text-4xl md:text-5xl font-display text-gold-gradient">4.9</span>
          <div className="flex flex-col items-start gap-1">
            ...
            <span className="text-xs font-heading tracking-widest text-[#6B5A3E] uppercase">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-2 md:grid-cols-4 gap-10">
        {trustBadges.map((badge) => (
          <div key={badge.title} className="flex flex-col items-center text-center gap-4">
            ...
            <div>
              <h4 className="font-heading font-bold text-sm md:text-base uppercase tracking-wider text-gold mb-1.5">
                {badge.title}
              </h4>
              <p className="text-[#6B5A3E] font-body text-xs md:text-sm leading-relaxed max-w-[220px]">
                {badge.desc}
              </p>
            </div>
          </div>
