...
export function SearchOverlay() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 sm:pt-24 md:pt-32 px-3 sm:px-4">
      <div
        className={`absolute inset-0 bg-[#2D2400]/50 backdrop-blur-md transition-opacity duration-200 ${
      ...
      <div
        className={`relative w-full max-w-3xl bg-white border border-[#E8D5B0] rounded-2xl shadow-2xl overflow-hidden transition-all duration-200 ${
          isClosing ? 'opacity-0 scale-95' : 'opacity-100 scale-100 animate-in fade-in zoom-in-95'
        }`}
      >
        <div className="flex items-center p-3 sm:p-4 border-b border-[#E8D5B0]">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none text-[#2D2400] text-base md:text-xl px-3 md:px-4 py-2 focus:outline-none placeholder:text-[#6B5A3E]/50 font-heading min-w-0"
          />
          <button
            onClick={handleClose}
            aria-label="Close search"
            className="p-2 text-gold hover:text-gold-bright transition-colors rounded-full hover:bg-[#FAF3E8] shrink-0"
          >
        ...
        <div className="max-h-[60vh] overflow-y-auto">
          {query.trim() === '' ? (
            <div className="p-5 sm:p-6 bg-[#FAF3E8]/80">
              ...
              <div className="flex flex-wrap gap-2">
                {POPULAR_SEARCHES.map((term) => (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-full border border-[#E8D5B0] text-[#6B5A3E] text-xs sm:text-sm hover:border-gold hover:text-gold transition-colors"
                  >
                    {term}
                  </button>
                ))}
            ...
            <div className="p-8 md:p-10 flex flex-col items-center text-center bg-[#FAF3E8]/80 animate-in fade-in duration-200">
              <SearchX className="w-8 h-8 md:w-10 md:h-10 text-gold/40 mb-3" />
              <p className="font-heading text-[#2D2400] mb-1 text-sm md:text-base">No results for "{query}"</p>
              <p className="text-xs md:text-sm text-[#6B5A3E]/60">Try a mukhi number, mala type, or an intention like "prosperity".</p>
            </div>
          ) : (
            <div className="p-3 sm:p-4 bg-[#FAF3E8]/80 animate-in fade-in duration-200">
              ...
              <div className="flex flex-col gap-1">
                {results.map((product) => (
                  <button
                    key={product.id}
                    className="flex items-center gap-3 md:gap-4 p-2 rounded-xl hover:bg-[#FAF3E8] transition-colors text-left group"
                  >
                    <div className="w-12 h-12 md:w-14 md:h-14 rounded-lg overflow-hidden border border-[#E8D5B0] shrink-0">
                      <img
                        src={product.image}
                        alt={product.name}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-heading text-[#2D2400] text-xs md:text-sm font-semibold truncate group-hover:text-gold transition-colors">
                        {product.name}
                      </p>
